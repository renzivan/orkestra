...
**Agent**:
A configured actor that performs one step of work. Composed of an identity, a base instruction (its core, always-on behavior — like a CLAUDE.md/AGENTS.md), zero or more skills layered on top, zero or more projects it works on, and an adapter + model + thinking effort it runs on. When an agent runs, all its project paths are made available in context; there is no single working directory.
...
**Adapter**:
A built-in preset describing how to spawn a local CLI (e.g. `claude`), defined in code and made available only when its executable is found on PATH. Its command template's placeholders — the system text (base instruction + skills), the input, the model, the effort, and the agent's project paths — are filled to build the command Orkestra runs. An agent uses one adapter. Not user-authored: there is no adapter UI; presets are detected, not entered.
